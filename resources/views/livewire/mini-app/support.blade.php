<div>
    ...
    <div class="bg-neutral-800 border border-neutral-700 rounded-xl p-1 pt-4 mt-4  relative">
        <ul wire:poll.5s
            class="space-y-3 flex flex-col-reverse px-1 h-[calc(100vh-310px)] overflow-y-scroll [&::-webkit-scrollbar]:w-1
                ...
                @foreach ($messages as $item)
                    @if ($item->is_admin)
                        <li class="max-w-60 flex gap-x-2 sm:gap-x-4 me-11">
                            <div>
                                <div
                                    class="bg-white text-sm text-white rounded-2xl px-4 py-2 space-y-3 dark:bg-neutral-900">
                                    {{ $item->message }}
                                </div>
                                <span
                        ...
                        <li class="flex ms-auto gap-x-2 sm:gap-x-4">
                            <div class="grow text-end space-y-3">
                                <div class="inline-flex flex-col justify-end">
                                    <div
                                        class="text-sm text-white inline-block bg-blue-800 rounded-2xl py-2 px-4 shadow-2xs">
                                        {{ $item->message }}
                                    </div>
        ...
        <form wire:submit="add_message"
            ...
            <input wire:model="message" type="text"
                class="h-full w-full bg-transparent border-0 outline-0 text-white" placeholder="Выедите сообщения здесь"
                required>
            <button type="submit"
                class="absolute top-0 right-0 bg-blue-500 h-full px-3 text-white font-semibold">Отправить</button>
        </form>
    </div>
</div>
